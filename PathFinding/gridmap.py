...
class OccupancyGridMap:
    def __init__(self, data_array, data_array_original, cell_size, occupancy_threshold=0.8):
        """
        Creates a grid map
        :param data_array: a 2D array with a value of occupancy per cell (values from 0 - 1)
        :param data_array_original: the non modified 2D array (avec les bordures d'origine pour le traçage)
        ...
        self.data_origine = data_array_original  # uniquement pour le traçage
    ...
    def plot(self, alpha=1, min_val=0, origin='lower'):
        ...
        plot the grid map
        """
        plt.imshow(self.data_origine, vmin=min_val, vmax=1, origin=origin, interpolation='none', alpha=alpha)
        plt.draw()
    ...
    def from_png(filename, cell_size, dilatation_iterations = 10):
        ...
        ogm = OccupancyGridMap(ogm_data_arr_dilated, ogm_data_arr, cell_size)
